...
exports.onUpdateProjectTitle = functions.firestore.document(`${CONSTANTS.DB.PROJECTS}/{projectId}`).onUpdate((change, context) => {
    const newValue = change.after.data()
    const prevValue = change.before.data()

    if (newValue[CONSTANTS.DB.TITLE] === prevValue[CONSTANTS.DB.TITLE]) {
        return null;
    }
