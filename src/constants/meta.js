export const SLOGAN = "The fastest way to make a checklist with friends";
export const TITLE = `NMBO - ${SLOGAN}`
export const DESCRIPTION = "Create a checklist and share the link, and anyone can start contributing. It's free to use. No account or download needed. ";
export const SIGN_IN_TITLE = "Log in | NMBO";
export const SIGN_UP_TITLE = "Sign up | NMBO";
export const ABOUT_TITLE = "About | NMBO";
export const FAQS_TITLE = "FAQs | NMBO";
export const PRIVACY_POLICY_TITLE = "Privacy Policy | NMBO";
export const TERMS_OF_SERVICE_TITLE = "Terms of Service | NMBO";
export const SETTINGS_TITLE = "Settings | NMBO";
export const EDIT_PROFILE_TITLE = "Edit Profile | NMBO";
export const ACCOUNT_SETTINGS_TITLE = "Account Settings | NMBO";
export const PROJECTS_TITLE = "Checklists | NMBO"; 
export const PROJECT_TITLE = (projectTitle) => `${projectTitle} | NMBO`;
export const IMAGE = "https://i.imgur.com/gQ3azKV.png";
export const URL = "https://nmbo.app";
export const CHECKLIST_DOESNT_EXIST = "Checklist doesn't exist | NMBO";
export const LOGO = "https://i.imgur.com/nLKnY4g.png";